...
def profile_settings(request):
    ...
    # Password change with OTP
        ...
        otp = password_form.data.get('otp')
        if not otp:
            # On first POST, send OTP and do not change password
            if password_form.is_valid():
                ...
                otp_required = True
                error_message = 'OTP sent to your email. Please enter it to confirm password change.'
            else:
                error_message = 'Please correct the errors below.'
        else:
            # On second POST, require and validate OTP before changing password
            if password_form.is_valid():
                otp_obj = OTP.objects.filter(user=user, otp=otp, is_verified=False).first()
                if otp_obj:
...
def admin_donation_blood_request(request):
    ...
    # Donation Requests
    donation_requests = DonationAppointment.objects.select_related('donor', 'center').all()
    if not history:
        if status_filter:
    ...
    # Blood Requests
    blood_requests = BloodRequest.objects.select_related('patient').all()
    if not history:
        if status_filter:
...
def approve_blood_request(request, pk):
    ...
    # Only approve if enough units AND at least 10 units will remain after approval
    if not stock or stock.units < req.quantity or (stock.units - req.quantity) < 10:
        req.status = 'REJECTED'
        req.save()
        Notification.objects.create(
            user=req.patient,
            title='Blood Request Rejected',
            message=f'Your blood request for {req.blood_group} ({req.quantity} units) was rejected due to low stock (at least 10 units must remain after approval).',
            type='blood_request'
        )
        return redirect('admin_donation_blood_request')
